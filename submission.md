...
2. **How you reproduced it:** I wrote `tests/test_search.py` with a fixture seeding songs with zero, one, and three tags, since `search_songs()` in `search_service.py` joins `Song` to `song_tags`, and joining to a many-side table is a classic source of duplicate rows when a row has multiple matches. All 5 tests I wrote passed on the first run, including `test_search_no_duplicates_multi_tag_song` (a song with 3 tags), which was surprising given the join looked suspicious.

   To understand why, I wrote a standalone debug script (`debug_search.py`) that ran the exact query from `search_songs()` directly and printed the generated SQL and raw result count. Against a song with 3 confirmed `song_tags` rows, the query's `.all()` still returned exactly 1 `Song` object. I then systematically tested every hypothesis I could think of for what *would* cause duplication:
   - **Selecting `Song` plus a raw column from the joined table** (`query(Song, song_tags.c.tag_id)`) — this *did* reproduce 3 separate result tuples.
   - **Filtering through a `Tag` join by tag name** — did not reproduce; still 1 result.
   - **A query matching both title and artist of the same song via `db.or_()`** — did not reproduce; still 1 result (SQL `OR` evaluates per-row, not as two merged queries).
   - **Two genuinely separate `Song` rows sharing the same title/artist** — did reproduce 2 results, but this is arguably correct behavior (two distinct database rows are two distinct songs), not a search bug.

   I also ran `search_songs()` against the actual seeded database (via `seed_data.py`, which explicitly comments that its multi-tag songs "expose Issue #3") using the same app factory the real app uses. The result was still exactly 1 for the seeded multi-tag song "Crown Heights Anthem," not 3.

3. **How you found the root cause:** I traced why the "obvious" join-multiplication theory didn't hold by inspecting the generated SQL (`print(query)`) and comparing it against a mixed-column query. The `SELECT` clause in `search_songs()` only selects `Song` columns — never anything from `song_tags`. SQLAlchemy's ORM deduplicates identical entities when a query selects only one entity type (`query(Song)`), even if the underlying SQL join produces multiple raw rows per entity; it only fails to deduplicate when the query also selects columns from the joined table directly. Since `search_songs()` never does that — its `.filter()` only references `Song.title`/`Song.artist`, and its `.outerjoin(song_tags, ...)` result is never otherwise used — the join has no way to actually produce duplicates in this function as written.

   I also checked whether any route or service ever calls `search_songs()` with additional filtering logic that might change the query shape (`routes/songs.py`'s `search()` only passes through the `q` query param) and whether `Song` rows could be created more than once anywhere in the app (only `seed_data.py` and test fixtures construct `Song(...)`; there is no "share a song" route that could double-insert one).

4. **The root cause:** I could not reproduce Issue #3 against the code as currently written. The `.outerjoin(song_tags, Song.id == song_tags.c.song_id)` in `search_songs()` is unused dead code — it doesn't affect filtering (the `WHERE` clause never references `song_tags`) and doesn't affect the result shape (`Song.to_dict()` already fetches tags independently via the model's own `tags` relationship). Because the query only selects `Song` entities, SQLAlchemy's ORM-level deduplication prevents the join from producing duplicate rows in the returned list, regardless of how many tags a song has. `seed_data.py` contains a comment claiming its multi-tag songs "expose Issue #3," but this did not hold up when tested directly against the real seeded database. I'm documenting this as a non-reproducible issue under the current codebase rather than forcing a change onto code that isn't actually causing the reported symptom.

5. **Your fix and side-effect check:** Since I could not reproduce the duplication, I did not change the search *logic*. As a cleanup (not a behavioral fix), I removed the unused `.outerjoin(song_tags, ...)` from `search_songs()`, since it serves no purpose and could be mistaken for load-bearing code in the future. Re-running `pytest tests/test_search.py -v` after removing it showed all 5 tests still passing, confirming the join was truly unnecessary and its removal changes nothing about search behavior or output.
